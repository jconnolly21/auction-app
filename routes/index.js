...
router.get('/mybudget', function(req, res, next) {
  const client = new Client({
    connectionString: process.env.DATABASE_URL,
    ssl: true,
  });
  client.connect(function (err, client, done) {
      ...
      var queryString = 'SELECT * from mybudget ORDER BY positionid ASC';
  		client.query(queryString, (err, response) => {
  			if (err) {
  				console.log('Error querying db.');
  				throw err;
  			} else {
    			console.log('Succesful budget query.')
    			res.send({budget: response.rows});
    		}
    		client.end();
  		});
  	}
